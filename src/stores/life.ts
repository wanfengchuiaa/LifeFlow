...
export const useLifeStore = defineStore('life', () => {
  ...
  async function refresh() {
    const data = await readAll() as Record<string, any[]>
  ...
  async function addBudget(input: Omit<Budget, keyof ReturnType<typeof newBase>>) {
    ...
    if (current) await db.budgets.put({ ...current, amount: input.amount, updatedAt: nowIso() })
  ...
  async function toggleTask(task: Task) {
    ...
    await db.tasks.put({ ...task, status: completing ? 'done' : 'todo', completedAt: completing ? nowIso() : null, updatedAt: nowIso() })
  ...
  async function stopTimer() {
    if (!activeTimer.value) return
    const endAt = nowIso()
    await db.timeEntries.put({ ...activeTimer.value, endAt, durationMinutes: elapsedMinutes(activeTimer.value.startAt, endAt), updatedAt: nowIso() })
    await refresh()
  }
  ...
  async function login(username: string, password: string) { void username; void password; authenticated.value = true; await refresh() }
  async function logout() { authenticated.value = true; ready.value = true }
