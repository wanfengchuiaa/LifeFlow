...
async function saveGoals() { await store.updateSettings({ calorieTarget: Number(calorieTarget.value), targetWeightKg: Number(targetWeight.value) }); notify('目标设置已更新') }
...
<template>
  <div class="page-content settings-page">
    <section class="page-intro"><div><span class="eyebrow">SETTINGS</span><h2>设置与本地数据</h2><p>LifeFlow 不上传任何个人记录，数据完全保存在当前浏览器。</p></div><span class="privacy-badge"><ShieldCheck :size="18" />本地优先</span></section>
    <section class="settings-grid">
      <article class="panel setting-section"><header><span class="setting-icon green"><RefreshCw :size="20" /></span><div><h3>健康目标</h3><p>用于首页和健康页的每日进度计算。</p></div></header><div class="form-grid"><label>每日热量目标（kcal）<input v-model="calorieTarget" type="number" min="500" step="50" /></label><label>目标体重（kg）<input v-model="targetWeight" type="number" min="20" step="0.1" /></label></div><button class="button primary compact" @click="saveGoals">保存目标</button></article>
